...
function Hero() {
  ...
  const { name, title, tagline, email, whatsapp, mediaType = 'image', videoUrl = '', videoOrientation = 'vertical' } = content.hero

  const renderMedia = () => {
    if (mediaType === 'video' && videoUrl) {
      const isVertical = videoOrientation === 'vertical'
      const wrapClass = isVertical ? styles.heroVideoPhone : styles.heroVideoLandscape
      const isEmbed = videoUrl.includes('youtube') || videoUrl.includes('youtu.be') || videoUrl.includes('vimeo') || videoUrl.includes('instagram')
      return (
        <div className={wrapClass}>
          {isEmbed
            ? <iframe src={videoUrl} allowFullScreen allow="autoplay; encrypted-media" title="video" />
            : <video src={videoUrl} autoPlay muted loop playsInline />
          }
        </div>
      )
    }
    return <img src={heroImg} alt={name} className={styles.heroImage} />
  }
    ...
    <section id="hero" className={styles.hero}>
      <div className={styles.heroInner}>
        <div className={styles.heroText}>
          <span className={styles.heroBadge}>✦ Marketing Digital</span>
          ...
          <div className={styles.heroButtons}>
            <a href={`mailto:${email}`} className={styles.heroBtn}>
            ...
            <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noopener noreferrer" className={styles.heroBtnWhatsapp}>
